<div class="form-group">
	<label class="col-md-3 control-label text-left"><?php echo __('HTML Content');?></label>
	<div class="col-md-6">					
	</div>
</div>
...
<div class="form-group">
	<label class="col-md-3 control-label text-left"><?php echo __('Background Image');?></label>
	<div class="col-md-6">					
	</div>
</div>
<div class="form-group">
	...
	<div class="col-md-12 file-browser">
		...
		<?php foreach($this->data['params']['images'] as $k => $v) { ?>	
			<li id="file_<?php echo $v;?>" class="files" data-index="<?php echo $k;?>">
				<?php echo $this->Html->link('<span class="badge" data-badge="file_'.$k.'" data-file="'.$v.'">X</span>', array('controller' => 'modules', 'action' => 'functions', 'module' => 'mod_header', 'function' => 'remove', 'admin' => 'admin', 'plugin' => false, 'id' => $this->data['Module']['id']), array('escape' => false, 'class' => 'badge-wrapper'));?>
				<?php echo $this->Resize->image('backgrounds' . DS . $v, 200, 200, true);?>
			</li>
		<?php } ?>
		</ul>
		<?php } ?>
...
<?php echo $this->Form->input('params.in_content', array('div' => 'form-group', 'class' => 'select2-select-00 col-md-12 full-width-fix', 'label' => array('text' => __('Show image inline with content'), 'class' => 'col-md-2 control-label'), 'between' => '<div class="col-md-10">', 'after' => '</div>', 'options' => array('0' => 'No', '1' => 'Yes')));?>
<?php echo $this->Form->input('params.img_side', array('div' => 'form-group', 'class' => 'select2-select-00 col-md-12 full-width-fix', 'label' => array('text' => __('Push image to side'), 'class' => 'col-md-2 control-label'), 'between' => '<div class="col-md-10">', 'after' => '</div>', 'empty' => '', 'options' => array('left' => 'Left side of content', 'right' => 'Right side of content')));?>
...
$(window).load(function() {
	...
	var sortablefiles = function(){
		$('.images').sortable({ 
			update: function(e, u){ 
				$.post('".$this->Html->url(array('controller' => 'modules', 'action' => 'functions', 'admin' => 'admin', 'module' => 'mod_header', 'function' => 'reorder', 'id' => $this->data['Module']['id']))."', 
	...
	$('#upload-wrapper').mfupload({
		...
		maxsize		: 3000,
		post_upload	: '" . $this->Html->url(array('controller' => 'modules', 'action' => 'functions', 'admin' => 'admin', 'module' => 'mod_header', 'function' => 'upload', 'id' => $this->data['Module']['id'])) . "',
		folder		: folder,
